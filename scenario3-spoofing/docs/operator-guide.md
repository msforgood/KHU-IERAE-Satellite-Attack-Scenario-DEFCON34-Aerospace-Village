# Operator Guide — Scenario 3 "Telemetry Spoofing"
...
Message of the demo: **"A satellite can be failing while every screen says it's fine."**
Phases ①②③ are Scenario 2: a visitor forges a reaction-wheel torque command and
uplinks it; the victim ground station escalates to **ENERGY SUPPLY CRITICAL** and the
physical solar panel spins. Then **phase ④** — a rogue drone impersonates the satellite
and replays a forged "all nominal" beacon, so the dashboard **flips back to NOMINAL and
the alarm is suppressed** while the satellite keeps tumbling (the physical motors never
stop). No real RF, and the drone is not physically wired — both the uplink and the spoof
are software-simulated.

## Phase ④ — drone spoof (Scenario 3)

After TRANSMIT (phase ③) has raised the alarm, engage the spoof:

- **In the attacker UI:** the targeting console shows a floating **"④ DRONE SPOOF → HIDE
  THE ALARM"** button → opens the drone console (served at `:8000/drone`). Press
  **START SPOOFING**; the embedded victim dashboard flips **RED → GREEN** on the spot.
- **By hand (headless):**
  ```
  curl -X POST http://<GS>:4540/api/spoof -d '{"on":true}'    # alarm hidden (NOMINAL)
  curl -X POST http://<GS>:4540/api/spoof -d '{"on":false}'   # restore real telemetry
  ```

**What stays true under the spoof:** `/api/state` (the Arduino bridge / physical motors)
always returns the *real* tumbling state — the panel keeps spinning. Only the dashboard
WebSocket is rewritten to the drone's forged NOMINAL beacon (battery ~98%, SUN-TRACKING,
COMM CONNECTED). The deception lives only on the operator's screen; nothing about the
attack is undone. Talking point: *"The operator's console just lied to them — this is why
authenticated, tamper-evident telemetry matters."*
...
```
curl -X POST http://localhost:4540/api/reset      # return the GS to nominal (also clears the drone spoof)
```
The Command Builder is stateless — no reset needed (a refresh is optional). `reset`
clears the phase-④ spoof too, so the next visitor starts from a truthful dashboard.
