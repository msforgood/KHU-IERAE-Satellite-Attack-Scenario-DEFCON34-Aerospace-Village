#!/bin/sh
# In-container: preconfigure gpredict (ENIGMA-1 + OpenVSA rotator), run it under
...
#
# Time control (scenario-2): control.py (:6079) places ENIGMA-1 LEAD seconds before
...
#
# TLE registration: local /config/enigma1.tle → gpredict DB (satdata/<cat>.sat) +
# a module, so ENIGMA-1 is trackable on launch (no manual New Module step).
set -e
...
if [ -d /config ]; then
  ...
  # local TLE → gpredict .sat DB entry (registers ENIGMA-1) + module
  if [ -f /config/enigma1.tle ]; then
    ...
    C=$(sed -n '2p' /config/enigma1.tle | cut -c3-7 | tr -d ' ')
    [ -n "$C" ] && CAT="$C"
