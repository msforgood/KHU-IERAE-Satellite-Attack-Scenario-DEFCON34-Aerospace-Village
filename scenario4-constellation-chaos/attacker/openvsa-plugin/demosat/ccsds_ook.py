...
def build_payload(command, params):
    ...
    if command == "orbit_maneuver":
        # Target orbit elements (big-endian int16): altitude (km), inclination and
        # RAAN (deg x10 for 0.1 deg resolution). A station-keeping tweak is tiny;
        # a large change swings the orbit onto a neighbouring satellite.
        alt  = max(-32768, min(32767, int(round(float(p.get("altKm", 600))))))
        inc  = max(-32768, min(32767, int(round(float(p.get("inc", 0)) * 10))))
        raan = max(-32768, min(32767, int(round(float(p.get("raan", 0)) * 10))))
        return struct.pack(">hhh", alt, inc, raan), [
            {"name": "altitude",    "value": alt,        "unit": "km"},
            {"name": "inclination", "value": inc / 10.0, "unit": "deg"},
            {"name": "raan",        "value": raan / 10.0, "unit": "deg"},
        ]
