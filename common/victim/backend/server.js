...
const ATTACK_DELAY_MS = +(process.env.ATTACK_DELAY_MS || 100);
...
function handleUplink(msg) {
  ...
  browserWss.broadcast(
    JSON.stringify({
      type: "uplink",
      command,
      payload,
      at: msg.timestamp || null,
      rejected: !!(res && res._rejected),
      reason: (res && res._rejectMessage) || null,
    }),
  );
}
...
const httpServer = http.createServer((req, res) => {
  ...
  if (url === "/api/spoof" && req.method === "POST") {
    ...
    req.on("end", () => {
      ...
      try {
        ...
        if (j.on === false) on = false;
      } catch {}
      spoofing = on;
      console.log(
        `[spoof] drone beacon ${on ? "ACTIVE — GS alarm SUPPRESSED (satellite still tumbling)" : "cleared — real telemetry restored"}`,
      );
      broadcastState(); // flip the dashboard immediately
      res.writeHead(200);
      res.end(JSON.stringify({ spoofing }));
    });
    return;
  }
  ...
  if (url === "/api/inject" && req.method === "POST") {
    ...
    req.on("end", () => {
      try {
        handleUplink({ type: "uplink-command", ...JSON.parse(body || "{}") });
        res.writeHead(200);
        res.end("{}");
      } catch (e) {
        res.writeHead(400);
