...
export function createRotatorScene({ container, store, antennaTypes }) {
  ...
  // ── Antenna lock (drives the physical booth antenna via arduino_bridge.py) ─
  // "locked" = GPredict is tracking AND the antenna is aimed at the satellite
  // (beam attenuation near boresight). Binary. Hysteresis + a hold timer keep
  // the model antenna from twitching at the beam edge; emitted only on change
  // as {type:"antenna-lock", locked} → server.js → bridge → Arduino 'L'/'U'.
  const LOCK_ON_DB = -3, LOCK_OFF_DB = -6, LOCK_HOLD_MS = 1000;
  let antennaLocked = false, lockCandidate = false, lockCandidateSince = 0;

  function updateAntennaLock(beamAtten) {
    const gpConnected = !!store.getState().gpredictConnected;
    // beamAtten is -60 when the satellite is below the horizon / untracked,
    // so a threshold near 0 dB also enforces "visible and aimed".
    const aimed = antennaLocked ? (beamAtten > LOCK_OFF_DB) : (beamAtten > LOCK_ON_DB);
    const raw = gpConnected && aimed;
    const now = Date.now();
    if (raw !== lockCandidate) { lockCandidate = raw; lockCandidateSince = now; }
    if (lockCandidate !== antennaLocked && now - lockCandidateSince >= LOCK_HOLD_MS) {
      antennaLocked = lockCandidate;
      store.setState((s) => ({ ...s, antennaLocked }));
      window.dispatchEvent(new CustomEvent("antenna-lock", { detail: { locked: antennaLocked } }));
    }
  }
