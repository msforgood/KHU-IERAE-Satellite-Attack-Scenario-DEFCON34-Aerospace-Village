...
```bash
# 0) (선택) 시나리오1 VSA 를 먼저 띄우면 gpredict 로테이터가 붙는다
...
- **안테나 추적(Antenna Control)**: 모듈 ≡ 메뉴 ▸ **Antenna Control** → 로테이터 `OpenVSA`(host:4533) 로드,
  **Track** 로 ENIGMA-1 자동추적, **Engage** 로 VSA rotctld 연결(VSA `node server.js` 가 켜져 있어야 함).
...
`../gpredict-config/OpenVSA.rig`(gpredict 형식 `[Radio]` 그룹)를 host 의 VSA rigctld(:4532)에 연결하도록
자동 등록. 모듈 ≡ 메뉴 ▸ **Radio Control** ▸ Device `OpenVSA` ▸ Engage → gpredict 가 궤도에서 계산한
**Doppler 보정 주파수를 VSA rigctld 로 전송**(다운링크 RX 튜닝). VSA `node server.js` 필요.
...
| `ROTCTLD_HOST` | host.docker.internal | 호스트의 VSA rotctld(:4533)/rigctld(:4532) 주소 |
